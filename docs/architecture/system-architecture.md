...
`catalog_reindex.py` 负责固定顺序：读取两个目录工作簿的档号集合和档号名称工作簿、按档号自然顺序
...
应用服务的图片计划保留页码、源路径、目标路径和本次文件题名；PDF 分组复用该计划，不从已生成 PDF 或图片旧题名反推业务数据。同一工作表内完全相同题名的所有图片按页码升序生成一个 PDF，以该组最小页码作为三位起始页前缀，不跨文件夹合并。
